...
/// return the band-2 `Rect` for the active panel to draw into — wrapped in a
/// thin dim border, so callers draw *inside* it. `footer` is a slice of
/// `(key, label)` pairs joined by ` · ` with keys accented.
