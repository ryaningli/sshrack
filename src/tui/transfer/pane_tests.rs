...
fn new_starts_empty_with_no_query_no_marks() {
    ...
    assert_eq!(p.core.cwd, PathBuf::from("/x"));
    assert!(p.core.entries.is_empty());
    assert!(p.core.query.is_empty());
    assert!(p.core.ranked.is_empty());
...
fn set_entries_resets_cursor_to_zero_and_ranks_all() {
    let cwd = PathBuf::from("/x");
    let mut p = Pane::new(cwd.clone());
    p.core.selected = 7; // pretend a stale cursor
    p.set_entries(vec![
        entry("apple", &cwd, false),
        entry("banana", &cwd, false),
    ]);
    assert_eq!(p.core.selected, 0, "cursor reset to 0");
    assert_eq!(p.core.ranked.len(), 2, "both entries ranked");
    assert_eq!(p.core.ranked, vec![0, 1], "empty query keeps entry order");
}
...
fn set_entries_preserves_query_for_in_place_refresh() {
    ...
    assert_eq!(p.core.query, "an", "query preserved on in-place refresh");
    ...
    assert_eq!(p.core.selected, 0, "cursor reset on refresh");
}
...
fn on_step_clears_marks_query_and_cursor() {
    ...
    assert!(p.core.marked.contains(&cwd.join("apple")));
    assert_eq!(p.core.query, "q");
    // Act: the screen is about to load a new dir.
    p.on_step();
    assert!(p.core.marked.is_empty(), "marks cleared");
    assert!(p.core.query.is_empty(), "query cleared");
    assert_eq!(p.core.selected, 0, "cursor reset");
}
...
fn typing_a_char_appends_to_query_and_filters() {
    ...
    assert_eq!(p.core.selected, 0, "cursor reset to 0 on query change");
}
...
fn down_then_up_moves_cursor_and_wraps() {
    ...
    assert_eq!(p.core.selected, 1);
    let _ = p.on_key(press(KeyCode::Down));
    assert_eq!(p.core.selected, 2);
    // wrap bottom → top
    let _ = p.on_key(press(KeyCode::Down));
    assert_eq!(p.core.selected, 0);
    // wrap top → bottom
...
fn space_on_file_toggles_mark_and_path_appears_in_marked() {
    ...
    assert!(p.core.marked.contains(&target), "marked after first Space");
    // Second Space untoggles.
    let out = p.on_key(press(KeyCode::Char(' ')));
    assert_eq!(out, PaneOutcome::ToggleMark(target.clone()));
    assert!(
        !p.core.marked.contains(&target),
        "unmarked after second Space"
    );
}
...
fn space_on_dir_toggles_mark() {
    ...
    let out = p.on_key(press(KeyCode::Char(' ')));
    assert_eq!(out, PaneOutcome::ToggleMark(target.clone()));
    assert!(p.core.marked.contains(&target));
}
...
fn visible_window_keeps_cursor_centered_then_clamps_to_tail() {
    ...
    assert_eq!(p.core.ranked.len(), 20);
    // Move cursor to 15; window of 5 → focus_window(20, 15, 5) = 13..18.
    for _ in 0..15 {
        let _ = p.on_key(press(KeyCode::Down));
    }
    assert_eq!(p.core.selected, 15);
    let win = p.visible_window(5);
    assert!(
        win.contains(&p.core.selected),
        "{}..{} excludes {}",
        win.start,
        win.end,
        p.core.selected
    ...
    assert_eq!(p.core.selected, 19);
    let win = p.visible_window(5);
    assert!(win.contains(&p.core.selected));
    assert_eq!(win, 15..20);
}
...
fn non_press_events_emit_none_and_do_not_mutate() {
    ...
    assert!(p.core.query.is_empty(), "release did not append to query");
}
...
fn set_entries_without_on_step_resets_cursor_to_zero() {
    ...
    p.core.selected = 1; // cursor on banana
    p.set_entries(vec![
        entry("apple", &cwd, false),
        entry("banana", &cwd, false),
        entry("cherry", &cwd, false),
    ]);
    assert_eq!(p.core.selected, 0, "in-place refresh resets cursor to 0");
}
...
fn step_into_and_back_restores_cursor() {
    ...
    assert_eq!(p.core.selected, 0, "first visit to /A/B2 lands at 0");
