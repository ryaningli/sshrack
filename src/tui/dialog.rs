...
const MAX_W: u16 = 80;
/// Max dialog outer height. `pub(crate)` so the queue-overlay list-sizing
/// path in [`transfer`][super::transfer] can derive its list cap (MAX_H minus
/// border+footer+tab) and so the Help render path can clamp its body.
