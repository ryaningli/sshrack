...
Guidance for AI coding agents working in this repo — Claude Code, Cursor, Windsurf, Codex, GitHub Copilot, or any other tool. This file is the single canonical agent guide (tool-agnostic); `CLAUDE.md` is only a thin pointer to it. **Long-form reference lives in `docs/`** (read on demand — not auto-loaded into context); this file keeps only high-frequency, non-obvious essentials.

## Building Blocks

- **Architecture** — workspace layout, core invariants, identity/config model, on-disk layout, PTY boundaries → `docs/architecture.md`
- **TUI structural design** — shell/tabs/overlays/wizards/file picker → `docs/tui.md`
- **SFTP transfer design** → `docs/sftp.md`
- **Migration from `sshrack-old`** → `docs/migration.md`
- **Release runbook** — `v*` tag → CI auto-publish; don't `cargo publish` locally (CN mirror) → `docs/release.md`
- **Dependency policy & Rust-skills routing** → `docs/dependency-policy.md`
...
`Ctrl-Enter` collapses to a bare `Enter` on many terminals, so it is only a hidden alias. Entry: `sshrack sftp <name>` or `Ctrl-T` on a host. Full SFTP architecture → `docs/sftp.md`.
...
Full detail (workspace tree, identity/config model, on-disk layout, PTY boundaries) → `docs/architecture.md`.
...
**Hard constraints carried from prior pain:**
...
SSH protocol libraries (`russh`, `ssh2`, `russh-sftp`), `age`, `ssh2-config` (keeps MSRV 1.88 and the surface small). Full policy + crate-evaluation checklist → `docs/dependency-policy.md`.
...
Branches: `feat/<desc>` · `fix/<desc>` · `refactor/<desc>`. Release steps → `docs/release.md`.
