//! End-to-end: a host's `Auth::Ref` (by id) survives a credential alias rename.
//!
...
//! referenced credential's alias, and asserts [`credential::resolve`] still
...
/// credential's alias is renamed and the whole config is saved + reloaded.
///
/// The reference is by id, so the rename (which touches only `alias`) cannot
/// orphan it. The reloaded config must resolve to the credential's user and key
/// under the new alias.
#[test]
fn ref_by_id_survives_credential_alias_rename_across_disk() {
    ...
    let host = loaded.find_host_by_alias("web1").expect("host present");
    ...
    // Rename the credential's alias in the reloaded config and re-persist.
    let renamed = rename_credential_alias(&loaded, "team-dev", "prod-team").expect("rename");
    assert_ne!(
        loaded.credentials[0].alias, renamed.credentials[0].alias,
        "sanity: rename actually changed the alias"
    );
    ...
    let host = reloaded.find_host_by_alias("web1").expect("host present");
    ...
    assert_eq!(
        ...
        "user travels with the credential, not the alias"
    );
    assert_eq!(
        resolved.key_path.as_deref(),
        Some(std::path::Path::new("~/.ssh/team_ed25519"))
    );
    // The new alias is findable by alias, the old one is gone.
    assert!(reloaded.find_credential_by_alias("prod-team").is_some());
    assert!(reloaded.find_credential_by_alias("team-dev").is_none());
}
...
/// Return a new config with the named credential's alias changed to `new_alias`,
/// preserving the credential's id and body. Mirrors what `cred edit --rename`
/// produces at the pure-logic layer.
fn rename_credential_alias(
    cfg: &SshrackConfig,
    old_alias: &str,
    new_alias: &str,
) -> Option<SshrackConfig> {
    let mut next = cfg.clone();
    let cred = next.credentials.iter_mut().find(|c| c.alias == old_alias)?;
    cred.alias = new_alias.into();
    Some(next)
}
