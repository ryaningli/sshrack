...
mod tests {
    ...
    // `LIVE` is a process global, so parallel tests would race on it (one
    // test's `cleanup_all` drains another's entries). This serializer forces
    // the registry-touching tests to run one at a time without slowing the
    // rest of the suite. Lock is held for the whole test body.
