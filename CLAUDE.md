...
- **Screen (`tui/transfer/`):** a full-screen `App` view (not an `Overlay`) — two `Pane`s (local | remote), a progress/queue panel, a hotkey footer. `Tab` switches focus (direction = Upload when local focused, Download when remote focused); `Space` marks (current-directory scope, single-shot per enqueue); `Ctrl-Enter` enqueues marked/selected (dirs recurse via `get -R`/`put -R`); `Esc` cancels the in-flight transfer or closes; `Ctrl-C` closes. Same-name conflicts prompt overwrite/skip/overwrite-all/skip-all (downloads check the local target; uploads overwrite in place — remote-exists check deferred to a later phase).
