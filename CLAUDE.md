...
- **Identity-key source + inline paste (both host and credential add-edit wizards):** when `Secret = IdentityKey`, a **Source** row appears that cycles `Path ↔ Inline` with `←`/`→`. Under `Path` the existing single-line identity / certificate path fields are used. Under `Inline` two multiline paste areas render instead — **Privkey** (required) and **Cert** (optional) — and expand to a taller block when focused; `Enter` inserts a newline, `Tab`/`Shift-Tab` navigate fields, and `Ctrl-S` saves. The key text is **never echoed** on edit: the textareas start empty on every entry (including edit), so the existing inline key is never rendered; if the Privkey area is left blank on an edit, the original inline key is preserved unchanged (only a non-blank paste overwrites it).
