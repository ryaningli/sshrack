...
- **Modal dialog (`draw_dialog`)** — only for a failure or decision that needs a user *response* (confirm / choose / retry). Today: `HostKeyPrompt`, `CloseConfirm`, `QueueOverlay`. A failure whose only exit is "close the screen" has no response and must NOT use a dialog.
...
- A failure backed by a **raw subprocess/io string** (sftp stderr, `io::Error`) keeps an action-subject prefix (`"remote list failed: {msg}"`) because the raw string is not self-describing. If the `SshrackError` can degrade to a generic `Io` (`"io error"`), keep the prefix for the same reason.
