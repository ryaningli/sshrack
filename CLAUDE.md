...
- The pane filter box is path-aware: a plain name fuzzy-filters the current
  directory; any `/` triggers a drill find where intermediate segments match
  directory names **exactly** and only the final segment fuzzy-matches within
  the resolved directory (a trailing `/` lists that directory's contents).
